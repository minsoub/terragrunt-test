...
dependencies {
  paths = ["../markdown_test_vpc"]
}

dependency "markdown_test_vpc" {
  config_path = "../markdown_test_vpc"
}
...
inputs = {
  ...
  # type: string
  name = "MARKDOWN_TEST_SG"

  # ID of the VPC where to create security group
  # type: string
  vpc_id = dependency.markdown_test_vpc.outputs.vpc_id
